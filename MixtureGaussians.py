...
def gamma_fun(TotalClasses,mus,sigmas,pis):
    ...
    for clase in range(len(TotalClasses)):
        ...
        for element in range(TamañoClase):
            ...
            if denominador_gamma_array[element,clase]==0:
                gamma_probaility=0
            else:
...
def M_step(TotalClasses,N_k,gamma_k):
    ...
    NumberOfFeatures = len(TotalClasses[0][:,0])  #numero de features ex.2
    N = len(TotalClasses[0][0,:]) + len(TotalClasses[1][0,:])

    #Compute the news mu_k y pi_km  ->  mu_k=(1/N_k)* sum_i (Gamma_ik *x_i)   and pi= N_k/ Total num of data  ------------------------------
    mus=[]
    pis=[]
    for clase in range(len(TotalClasses)):
        current_class = TotalClasses[clase]
        pis.append(N_k[clase] / N)
        mu_k = np.mean(current_class, axis=1)
        mus.append(mu_k)
    ...
    for clase in range(len(TotalClasses)):
        current_class = TotalClasses[clase]
        gamma_k_class = gamma_k[:, clase]
        sigma_k = np.cov(current_class, bias=True, aweights=gamma_k_class)
        sigmas.append(sigma_k)
...
def MixtureOfGaussians(train_data,y_label, NumberOfSteps=2):
    ...
    #--------------------------------------------------------------------------------
        ...
        prediction=predict(train_data, mus, sigmas)
        #metricas
...
def predict(winner_group_class_1_trans, mus, sigmas):
    gaussian_values=[]
    prediction=[]
    for i in range(2):
        mu=mus[i]
        ...
        determinante_sigma=np.linalg.det(sigma)
        inv_sigma=np.linalg.inv(sigma)
        current_x=winner_group_class_1_trans
        gaussian=(1 / np.sqrt((2 * np.pi) ** 2 * determinante_sigma)) * np.exp(
...
if __name__ == "__main__":
    ...
    prediction=predict(winner_group_class_1_trans, mus, sigmas)
